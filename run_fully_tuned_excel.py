...
YANDEX_DATASETS = [
    ...
    'jannis', 'adult', 'eye', 'higgs-small', 'otto']
BENCHMARK_DATASETS = [
...
def get_training_args():
    ...
    parser.add_argument("--output", type=str, default='result_excel_fully_tune')
    ...
    if args.ratio < 1:
         args.output += f'({args.ratio})'
    args.output = args.output + f'/{args.dataset}/{args.model}'
    ...
    # fully tuned config
    cfg_file = f'configs_excel_fully_tune(fixed_sv)/{args.dataset}/{args.model}/cfg.json'
...
"""Using few training samples"""
if args.ratio < 1:
    sample_id_file = DATA / dataset_name / f'split({args.ratio}).npy'
    if os.path.exists(sample_id_file):
        sample_ids = np.load(sample_id_file)
    else:
        N = dataset.size('train')
        sample_ids = np.random.permutation(N)[:math.ceil(N * args.ratio)]
        np.save(sample_id_file, sample_ids)
    dataset.y['train'] = dataset.y['train'][sample_ids]
    dataset.X_num['train'] = dataset.X_num['train'][sample_ids]
    if dataset.X_cat is not None:
        dataset.X_cat['train'] = dataset.X_cat['train'][sample_ids]
...
args.early_stop = 200 if args.ratio == 1 else 16
...
cfg['training'].update({"batch_size": batch_size, "eval_batch_size": val_batch_size, "patience": args.early_stop})
...
def train_predict(
    ...
    report_frequency = 1 if report_frequency == 0 else report_frequency
    """Training Args"""
